...
const SearchPage = () => {
    ...
    // Extract search term from the route parameters
    const searchTerm = params.query;

    useEffect(() => {
        if (searchTerm) {
            setPage(1); // Reset page number on new search
            fetchFilteredProducts(1, searchTerm); // Fetch initial page of search results
        }
    }, [searchTerm]);

    const fetchFilteredProducts = async (currentPage, term) => {
        setLoading(true);
        try {
            const response = await GlobalApi.searchProducts(term, currentPage);
            if (currentPage === 1) {
                setProductList(response.data.data); // Set initial products list
            } else {
                setProductList((prev) => [...prev, ...response.data.data]); // Append new products to list
            }
    ...
    const lastProductRef = useCallback((node) => {
        ...
        observer.current = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting && !loading && page < totalPages) {
                ...
                fetchFilteredProducts(page + 1, searchTerm);
            }
        });
